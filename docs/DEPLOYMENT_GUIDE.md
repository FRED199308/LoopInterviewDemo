# NCBA POS Integration – Kubernetes Deployment Guide
...
docker build -t ncba/pos-integration:1.0.0 .

# Tag as latest too
docker tag ncba/pos-integration:1.0.0 ncba/pos-integration:latest
```
...
docker push ncba/pos-integration:1.0.0
docker push ncba/pos-integration:latest
```

If you are using **minikube** (local cluster), skip the push and load the image directly:
```bash
minikube image load ncba/pos-integration:1.0.0
```

If you are using **kind**:
```bash
kind load docker-image ncba/pos-integration:1.0.0
```
...
```bash
# Build new image
docker build -t ncba/pos-integration:1.0.1 .
docker push ncba/pos-integration:1.0.1

# Rolling update (zero downtime)
kubectl set image deployment/pos-integration \
  pos-integration=ncba/pos-integration:1.0.1 \
  -n pos-integration
