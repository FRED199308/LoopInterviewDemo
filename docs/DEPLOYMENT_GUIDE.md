...
docker build -t  pos-integration:1.0.0 .

# Tag as latest too
docker tag pos-integration:1.0.0  loopdfsregistry.azurecr.io/pos-integration:latest
```
...
docker login loopdfsregistry.azurecr.io  # or your private registry

docker push  loopdfsregistry.azurecr.io/pos-integration:1.0.0
docker push  loopdfsregistry.azurecr.io/pos-integration:latest
```

If you are using **minikube** (local cluster), skip the push and load the image directly:
```bash
minikube image load loopdfsregistry.azurecr.io/pos-integration:1.0.0
```
...
```yaml
image: loopdfsregistry.azurecr.io/pos-integration:1.0.0
```
